...
def create_ordered_dict_for_metric_and_alpha(
    metric, alpha, ordered_rules, ordered_n_cands
):
    return {
        rule: {
            n_cands: load_file(
                f"./bt_profile_stats/{metric}/{n_cands:02d}/alpha_{alpha:.2f}/METRIC_{metric}__NCANDS_{n_cands}__ALPHA_{alpha:.2f}__TYPE_{rule}.json"
            )
...
def build_plot_for_metric_and_alpha(
    metric, alpha, ordered_rules, ordered_n_cands, rule_color_map, ax, y_label=""
):
